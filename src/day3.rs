...
// Add a new struct to store symbol vs nums matched.
// Finnaly iterate over all symbol sturcts to find 
// sum of all nums for each symbol
pub fn solve() {
    let ip = INPUT;
    let mut num_lines: Vec<Vec<NumLine>> = Vec::new();
    let mut symbols: Vec<Vec<usize>> = Vec::new();
    let mut total = 0;
    let lines = ip.split("\n");
    let last = lines.clone().enumerate().last().unwrap().0;
    for (i, line) in lines.enumerate() {
        let (num_line, symbol) = parse_line(line);
        num_lines.push(num_line);
        symbols.push(symbol);
        
        // println!(" {} Line: {}, symbols: {:?}, numlines: {:?}",i, total, symbols, num_lines);
        if i > 0 {
            total = total + get_part_numbers(&symbols, &num_lines, i - 1);
        }
        if i == last {
            total = total + get_part_numbers(&symbols, &num_lines, num_lines.len() - 1);
        }
    }
...
fn parse_line(line: &str) -> (Vec<NumLine>, Vec<usize>) {
    let mut num_lines: Vec<NumLine> = Vec::new();
    ...
    for (i, ch) in line.chars().enumerate() {
        if ch.is_digit(10) {
            ...
            if num_string.len() > 0 {
                let num_line = get_num_line(num_string.clone(), i);
                num_lines.push(num_line);
    ...
    return (num_lines, symbols);
}

fn get_num_line(num_str: String, end: usize) -> NumLine {
    let num: usize = num_str.parse::<usize>().unwrap();

    return  NumLine {
        num,
        start: end - num_str.len(),
        end: end - 1,
    };
}
...
fn get_part_numbers(
    ...
    num_lines: &Vec<Vec<NumLine>>,
    index: usize,
) -> usize {
    let symbol = symbols.get(index).unwrap();
    let mut parts = 0;
    let before = if index > 0 { num_lines.get(index - 1) } else { None };
    let current = num_lines.get(index);
    let after = num_lines.get(index + 1);

    for position in symbol.iter() {
        if let Some(num_line) = before {
            parts += get_total(num_line, *position)
        }
        if let Some(num_line) = after {
            parts += get_total(num_line, *position)
        }

        parts += get_total(current.unwrap(), *position);
    }

    println!("End of symbol");
    // println!("GET PART NUMS: {}, {}, current: {:?}, before: {:?}, after: {:?}", index, parts, current, before, after);
    return parts;
}

fn get_total(num_lines: &Vec<NumLine>, index: usize) -> usize {
    let mut total = 0;
    for num_line in num_lines.iter() {
        if num_line.start == index || num_line.start == index+1 {
            println!("\n\nAdd3 index: {}, num: {};", index, num_line.num);
            total += num_line.num;
        } else if num_line.end == index || num_line.end == index-1 {
            println!("\n\nAdd2 index: {}, num: {};", index, num_line.num);
            total += num_line.num;
        } else if num_line.start <= index && num_line.end >= index {
            println!("\n\nAdd1 index: {}, num: {};", index, num_line.num);
            total += num_line.num;
        }
    }
...
const INPUT: &str = ".....613...................................439............498.........................438......617....343.............942...................
